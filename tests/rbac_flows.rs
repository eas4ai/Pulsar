...
#[tokio::test]
async fn promote_user_assigns_author_role_and_inherited_permissions() {
    ...
    assert!(!promoted.has_permission_to("articles.delete").await.unwrap());
}

#[tokio::test]
async fn member_role_has_no_authoring_permissions() {
    let _harness = common::setup().await;
...
#[tokio::test]
async fn console_command_promotes_existing_user() {
    let _harness = common::setup().await;
    ...
    assert!(user.has_permission_to("articles.delete").await.unwrap());
}
